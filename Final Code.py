...
def draw_time(start_time, pause_time):
    ...
    return [font1.render(get_time(hours, minutes, seconds), True, (255, 0, 255)),
...
class labyrinth:
    ...
    def __init__(self, id):
        ...
        for f in range(10):
            for s in range(10):
                ...
                if not (f==0 and s > 6):
                    self.cells.append(
                        cell((x + 8, t, 25, 8), (x + 8, t + 33, 25, 8), (x, t + 8, 8, 25), (x + 33, t + 8, 8, 25)))
                x += 33
            x = 0
            t += 33
        ...
        # dependes on reference dots creat walls
        for j in range(0, 951, 33):
    ...
    def draw(self, goal):
        ...
        # pygame.draw.rect(screen, (0, 128, 255), pygame.Rect(695, 0, 300, 105))  # clock background
        bg_image = pygame.image.load("sea.jpg").convert_alpha()
        rect = bg_image.get_rect()
        rect.bottomleft = (240, 33)
        screen.blit(bg_image, rect)

        goal2 = pygame.image.load("smallfugu.png").convert_alpha()
        DEFAULT_IMAGE_SIZE = (33, 33)
        goal3 = pygame.transform.scale(goal2, DEFAULT_IMAGE_SIZE)
        rect = bg_image.get_rect()
        rect.topleft = (296, 296)
        screen.blit(goal3, rect)
        # pygame.draw.rect(screen, (0, 255, 0), goal)  # finish


id = 0
running = True
while running:  # parameters
    screen = pygame.display.set_mode((340, 340))
    done = False
    color = (255, 255, 255)  # color of the walls
    x = 16
    y = 16
    I = 25
    J = 16
    ...
    id += 1
    maze = labyrinth(id)
    goal = pygame.Rect(305, 305, 25, 25)
    victory = False
    defeat = False
    ...
    while not done:
        ...
        # the actual game
        if not victory and not defeat and not pause:
            move_up = True
            move_down = True
            move_left = True
            move_right = True
            ...
            if pressed[pygame.K_UP]:
                # checks if there is an overlap with the wall
            ...
            if pressed[pygame.K_w]:
                # checks if there is an overlap with the wall
                for m in maze.maze_walls:
                    player = pygame.Rect(I, J - speed, 10, 10)
                    if player.colliderect(pygame.Rect(m[0], m[1], m[2], m[3])):
                        move_up = False
                        break
                if move_up:
            ...
            if pressed[pygame.K_a]:
                player = pygame.Rect(I - speed, J, 10, 10)
                for m in maze.maze_walls:
                    if player.colliderect(pygame.Rect(m[0], m[1], m[2], m[3])):
                        move_left = False
                        break
            ...
            player1 = pygame.image.load("images1.png").convert_alpha()
            DEFAULT_IMAGE_SIZE_PIN = (31, 31)
            player1 = pygame.transform.scale(player1, DEFAULT_IMAGE_SIZE_PIN)
            rect = player1.get_rect()
            rect.center = (x, y)
            # pygame.draw.rect(screen, (255, 100, 0), pygame.Rect(x, y, 10, 10)) # pinquins
            screen.blit(player1, rect)

            player2 = pygame.image.load("images2.png").convert_alpha()
            player2 = pygame.transform.scale(player2, DEFAULT_IMAGE_SIZE_PIN)
            rect = player2.get_rect()
            rect.center = (I, J)
            pygame.draw.rect(screen, (255, 100, 0), pygame.Rect(I, J, 10, 10)) # pinquins
            # pygame.draw.rect(screen, (255, 100, 0), pygame.Rect(x, y, 10, 10))
            screen.blit(player2, rect)

            screen.blit(text[0], (240, 3))
            # rotates the screen
            t = time.time() - start - pause_time
            newscreen = pygame.transform.rotate(screen, int(t / 5) * 90)
            screen.fill((0, 0, 0))
        ...
        if victory or defeat:
            ...
            468 - (time_text.get_width() // 2), (248 - (time_text.get_height() // 2)) + victory_text.get_height()))
            screen.blit(reset, (468 - (reset.get_width() // 2),
                                (248 - (reset.get_height() // 2)) + victory_text.get_height() + time_text.get_height()))
